            ...
            <Card>
              <CardHeader className="pb-2 pt-3 px-4">
                <CardTitle className="text-xs font-medium flex items-center gap-2">
                  ...
                  Empresas com projetos concluídos no período ({completedMetrics.companies.length})
                </CardTitle>
              </CardHeader>
              <CardContent className="px-4 pb-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                  {completedMetrics.companies.map(company => {
                    const companyProjects = projects.filter(p => getProjectCompanyId(p) === company.id && p.status === "completed");
