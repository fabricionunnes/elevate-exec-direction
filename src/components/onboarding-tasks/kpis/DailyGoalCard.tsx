...
export const DailyGoalCard = ({
  ...
  return (
    <div className="space-y-4">
      {perKpiData.map((kpiBlock) => {
        const companyPercentage = kpiBlock.totalTarget > 0
          ? (kpiBlock.totalRealized / kpiBlock.totalTarget) * 100
          : 0;

        return (
          <Card key={kpiBlock.kpi.id} className="relative overflow-hidden border-0 shadow-xl">
            ...
            <CardHeader className="relative pb-3">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-base">Meta Diária — {kpiBlock.kpi.name}</CardTitle>
                    <p className="text-xs text-muted-foreground">
                      {dayInfo.remaining} dia{dayInfo.remaining !== 1 ? "s" : ""} útil{dayInfo.remaining !== 1 ? "eis" : ""} restante{dayInfo.remaining !== 1 ? "s" : ""}
                    </p>
                  </div>
                </div>
                {/* Only show toggles on the first card */}
                {kpiBlock.kpi.id === perKpiData[0].kpi.id && (
                  <div className="flex items-center gap-2 flex-wrap">
                    {[
                      ...
                      <div key={toggle.id} className="flex items-center gap-1.5 bg-muted/50 rounded-full px-2.5 py-1">
                        <Switch
                          id={`${toggle.id}-${kpiBlock.kpi.id}`}
                          checked={toggle.checked}
                          onCheckedChange={toggle.onChange}
                          className="scale-[0.65]"
                        />
                        <Label htmlFor={`${toggle.id}-${kpiBlock.kpi.id}`} className="text-[10px] cursor-pointer">
                          {toggle.label}
                        </Label>
                      </div>
                    ))}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
                <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.15 }}>
                  <div className="relative overflow-hidden rounded-xl border p-3 bg-card/80 backdrop-blur-sm text-center">
                    ...
                    <p className="text-base sm:text-lg font-bold mt-0.5">{formatValue(kpiBlock.totalTarget, kpiBlock.kpiType)}</p>
                  </div>
                </motion.div>
                ...
                <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }}>
                  <div className="relative overflow-hidden rounded-xl border p-3 bg-card/80 backdrop-blur-sm text-center">
                    ...
                    <p className="text-base sm:text-lg font-bold mt-0.5">{formatValue(kpiBlock.totalRealized, kpiBlock.kpiType)}</p>
                  </div>
                </motion.div>
                ...
                <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.25 }}>
                  <div className="relative overflow-hidden rounded-xl border p-3 bg-card/80 backdrop-blur-sm text-center">
                    ...
                    <p className="text-base sm:text-lg font-bold mt-0.5">{formatValue(kpiBlock.remaining, kpiBlock.kpiType)}</p>
                  </div>
                </motion.div>
                ...
                <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }}>
                  <div className="relative overflow-hidden rounded-xl border border-primary/20 p-3 bg-gradient-to-br from-primary/5 to-primary/10 text-center">
                    ...
                    <p className="text-base sm:text-lg font-extrabold text-primary mt-0.5">
                      {formatValue(kpiBlock.dailyGoal, kpiBlock.kpiType)}
                    </p>
              ...
              {kpiBlock.salespeopleData.length > 0 && (
                ...
                >
                  ...
                  <div className="space-y-1.5">
                    {kpiBlock.salespeopleData.map((sp, idx) => {
