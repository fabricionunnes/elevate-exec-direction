...
import { TrendingUp, Trophy, Flame, Sparkles, Pencil, Check, X, Star, History, Plus, Target, ChevronDown } from "lucide-react";
...
export const NorthStarMetricCard = ({ companyId }: Props) => {
  ...
  return (
    <div className="relative">
      <Card
        className={`relative overflow-hidden rounded-2xl border border-border/50 shadow-sm bg-gradient-to-br ${palette.gradient} opacity-90`}
      >
        ...
        >
          <img src={northStarImg} alt="North Star" width={28} height={28} loading="lazy" className="h-7 w-7 drop-shadow" />
          <span className="uppercase tracking-[0.2em] text-[11px] font-bold text-white/90 inline-flex items-center gap-1.5 flex-1 text-left">
            <Icon className="h-3.5 w-3.5" />
            North Star Metric
            {label && <span className="text-white/60 normal-case tracking-normal font-normal">· {label}</span>}
          </span>
          {targetCents > 0 && (
            <span className="text-xs font-bold text-white/90 mr-2">{pct}%</span>
          )}
          <ChevronDown className={`h-3.5 w-3.5 text-white/60 transition-transform duration-200 ${expanded ? "rotate-180" : ""}`} />
        </button>

        {expanded && <CardContent className="relative px-5 pb-6 pt-2 space-y-6 text-white">
          {/* Header completo */}
            ...
            <div className="flex items-center gap-4">
              <div className="relative shrink-0">
                <img
                  ...
                  height={72}
                  loading="lazy"
                  className="relative h-16 w-16 drop-shadow-[0_0_15px_rgba(255,215,0,0.5)]"
                />
                {reached && (
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="uppercase tracking-[0.25em] text-[11px] font-bold text-white/90 inline-flex items-center gap-1.5">
                    <Icon className="h-3.5 w-3.5" />
                    North Star Metric
                  </span>
                ...
                <h2 className="text-xl md:text-2xl font-black tracking-tight leading-tight mt-1 drop-shadow">
                  North Star Metric
                </h2>
                {label && <p className="text-sm text-white/80 mt-0.5 font-medium">{label}</p>}
              </div>
            </div>

            {targetCents > 0 && (
              <div className={`px-4 py-2.5 rounded-xl border ${palette.chip} backdrop-blur-md shadow`}>
                <p className="text-[10px] uppercase tracking-wider opacity-90 font-bold">Atingido</p>
                <p className="text-2xl font-black leading-none drop-shadow">{pct}%</p>
              </div>
            )}
          </div>
