...
const THINK_DT = 0.18 // s — lógica pesada roda a ~5,5 Hz (perf)
...
export default function CleaningLady() {
  ...
  const thinkAccRef = useRef(0) // acumulador do throttle da lógica pesada
  ...
  useFrame(({ clock }, delta) => {
    ...
    if (broomRef.current) {
      broomRef.current.rotation.z = -0.15 + Math.sin(clock.getElapsedTime() * 4.5) * 0.3
    }

    // PERF: a lógica pesada (varredura de salas, estados, rota, fala) roda a
    // ~5,5 Hz; só o MOVIMENTO acompanha o frame — fluido gastando bem menos.
    thinkAccRef.current += delta
    const think = thinkAccRef.current >= THINK_DT
    if (think) thinkAccRef.current = 0
    ...
    // Segue a rota (TODO frame — movimento suave)
  ...
  return (
    <>
      ...
      >
        ...
        {/* Vassoura SEGURADA na mão — cabo diagonal até o chão, cerdas em
            leque; o pivô (broomRef) fica na mão e balança pra "varrer". */}
        <group ref={broomRef} position={[0.3, 0.74, 0.06]} rotation={[0.5, 0, -0.16]}>
          {/* cabo de madeira */}
          <mesh position={[0, -0.5, 0]} castShadow>
            <cylinderGeometry args={[0.017, 0.02, 1.05, 8]} />
            <primitive object={broomMat} attach="material" />
          </mesh>
          {/* virola metálica */}
          <mesh position={[0, -1.0, 0]}>
            <cylinderGeometry args={[0.032, 0.028, 0.06, 8]} />
            <meshStandardMaterial color="#8d949c" metalness={0.6} roughness={0.4} />
          </mesh>
          {/* base + cerdas em leque */}
          <group position={[0, -1.08, 0]}>
            <mesh>
              <boxGeometry args={[0.05, 0.05, 0.24]} />
              <meshStandardMaterial color="#7a5a2b" roughness={0.8} />
            </mesh>
            {[-0.09, -0.045, 0, 0.045, 0.09].map((zz, i) => (
              <mesh key={zz} position={[0, -0.11, zz]} rotation={[(i - 2) * 0.12, 0, 0]} castShadow>
                <boxGeometry args={[0.035, 0.22, 0.03]} />
                <meshStandardMaterial color="#d9b25e" roughness={0.9} />
              </mesh>
            ))}
          </group>
        </group>
