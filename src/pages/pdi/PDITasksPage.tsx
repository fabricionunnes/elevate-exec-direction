...
export default function PDITasksPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setTasks(((tasksRes.data as any[]) || []).map((t) => ({ ...t, track_name: trackMap.get(t.track_id) || "—" })));
    setLoading(false);
  }, []);
  ...
  const resetForm = () => {
    setForm({ title: "", description: "", task_type: "practical", due_days: "", support_material_url: "", track_id: "" });
    setEditingTask(null);
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Gerencie tarefas de desenvolvimento</p>
        </div>
                  ...
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground">
                        Trilha: {task.track_name}
                        {task.due_days && ` • Prazo: ${task.due_days} dias`}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
