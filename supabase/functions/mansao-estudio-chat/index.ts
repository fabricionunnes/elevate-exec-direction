    ...
    for (let round = 0; round < 6; round++) {
      ...
      for (const t of toolUses) {
        ...
        try {
          ...
          else if (t.name === "generate_system") { await sb.from("mansao_estudio").update({ system_status: "generating", system_spec: t.input.spec || "" }).eq("id", id); row.system_status = "generating"; bg.push(genSystem(sb, id, String(t.input.spec || ""))); events.push("system"); out = { ok: true, status: "generating", url: `https://mansaoempreendedora.com.br/estudio/?app=${row.slug}` }; }
