  ...
  try {
    ...
    switch (action) {
      ...
      case 'list-instances-custom': {
        // List instances using user-provided Evolution API base URL + key.
        // This is used by the CRM "Importar do STEVO" modal to support multiple managers.
        ...
        const tryGetArray = async (path: string) => {
          for (const prefix of ROUTE_PREFIXES) {
            const fullUrl = `${baseUrl}${prefix}${path}`;
            console.log(`[evolution-api] [custom] GET ${fullUrl}`);

            try {
              const res = await fetch(fullUrl, { method: 'GET', headers: customHeaders });
              const text = await res.text();
              let parsed: any = null;
              try {
              ...
              if (res.ok && Array.isArray(parsed)) {
                console.log(`[evolution-api] [custom] Found instances with prefix "${prefix}" and path "${path}"`);
                return parsed;
              }

              // Continue on typical "route doesn't exist" codes.
        ...
        return new Response(
          JSON.stringify({
            error: 'Unable to list instances from custom Evolution API',
            hint: 'Verifique se a URL é a base da API (sem /manager) e se a API Key está correta.',
            lastStatus,
            lastBodyPreview,
            tried,
            _version: EVOLUTION_API_FUNC_VERSION,
      ...
      case 'set-webhook-custom': {
        // Set webhook using user-provided Evolution API base URL + key.
        ...
        for (const prefix of ROUTE_PREFIXES) {
          for (const { path, method } of endpointConfigs) {
            ...
            try {
              ...
              if (res.ok) {
                return new Response(
                  JSON.stringify({ ok: true, details: parsed, _version: EVOLUTION_API_FUNC_VERSION }),
                  { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
                );
              }
