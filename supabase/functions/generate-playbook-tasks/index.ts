...
// Feriados nacionais brasileiros (fixos e móveis para 2024-2026)
...
// Helper: Get next business day from a given date
...
// Helper: Get the next occurrence based on recurrence type starting from a date
function getNextOccurrence(startDate: Date, recurrence: "daily" | "weekly" | "monthly"): Date {
  ...
  // Adjust to next business day if falls on weekend/holiday
  return getNextBusinessDay(nextDate);
}

// Helper: Format month name in Portuguese
...
serve(async (req) => {
  ...
  try {
    ...
    // Ensure we start from a business day
    const projectStartDate = getNextBusinessDay(projectCreatedAt);
    let daysOffset = 0;

    // Calculate project duration (12 months from start)
    ...
    for (const phase of productConfig.phases) {
      ...
      for (const taskTitle of phase.tasks) {
        ...
        // Detect recurrence from task title
        const lowerTitle = taskTitle.toLowerCase();
        let recurrence: "daily" | "weekly" | "monthly" | null = null;
        
        if (lowerTitle.includes("diário") || lowerTitle.includes("diária") || lowerTitle.includes("diariamente") || lowerTitle.includes("todo dia")) {
          recurrence = "daily";
        } else if (lowerTitle.includes("semanal") || lowerTitle.includes("semanalmente") || lowerTitle.includes("toda semana") || lowerTitle.includes("check-in semanal")) {
          recurrence = "weekly";
        } else if (lowerTitle.includes("mensal") || lowerTitle.includes("mensalmente") || lowerTitle.includes("todo mês") || lowerTitle.includes("direção mensal") || lowerTitle.includes("board mensal") || lowerTitle.includes("reunião mensal")) {
          recurrence = "monthly";
        }

        // Calculate base due date for first task
        const phaseDuration = Math.ceil(365 / productConfig.phases.length);
        ...
        let baseDueDate = addBusinessDays(projectStartDate, taskDueDay);

        // If it's a recurring task, generate multiple instances
        if (recurrence) {
          let currentDate = getNextBusinessDay(projectStartDate);
          let instanceCount = 0;
          const maxInstances = recurrence === "daily" ? 60 : recurrence === "weekly" ? 52 : 12;

          while (currentDate < projectEndDate && instanceCount < maxInstances) {
            const instanceDueDate = getNextBusinessDay(currentDate);
            
            // Generate instance title with date reference
            let instanceTitle = personalizedTitle;
            if (recurrence === "daily") {
              instanceTitle = `${personalizedTitle} - ${instanceDueDate.toLocaleDateString("pt-BR")}`;
            } else if (recurrence === "weekly") {
              instanceTitle = `${personalizedTitle} - Semana ${getWeekNumber(instanceDueDate)}/${instanceDueDate.getFullYear()}`;
            } else if (recurrence === "monthly") {
              instanceTitle = `${personalizedTitle} - ${getMonthName(instanceDueDate.getMonth())}/${instanceDueDate.getFullYear()}`;
            }

            // Skip if this specific instance already exists
            if (!existingTaskTitles.includes(instanceTitle.toLowerCase())) {
              tasks.push({
                title: instanceTitle,
                description: `${phase.description} - ${productConfig.name}\n\nParte das entregas: ${productConfig.deliverables.slice(0, 3).join(", ")}...\n\nRecorrência: ${recurrence === "daily" ? "Diária" : recurrence === "weekly" ? "Semanal" : "Mensal"}`,
                phase: phase.name,
                priority,
                responsible_role,
                estimated_days: 1,
                due_date: instanceDueDate.toISOString().split("T")[0]
              });
            }
          ...
          // Non-recurring task - just add once
          tasks.push({
    ...
    // Limit to reasonable number of tasks (increased for recurring tasks)
    const limitedTasks = tasks.slice(0, 150);

    console.log(`Generated ${limitedTasks.length} tasks for ${productConfig.name} (including recurring instances)`);
