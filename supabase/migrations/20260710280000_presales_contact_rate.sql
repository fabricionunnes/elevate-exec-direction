-- Adiciona taxa de contato (% dos leads de horário comercial que receberam algum
-- toque) — par do "tempo de primeiro contato", mesma base (leads que entram no funil
-- seg-sex 08h-19h, fuso Brasília). Muda a assinatura de retorno → DROP + CREATE.
...
BEGIN
  ...
  -- Tempo de 1º contato E taxa de contato — mesma base: leads que entram no funil
  -- em HORÁRIO COMERCIAL (seg-sex, 08h-19h, fuso de Brasília).
  SELECT
    round(avg(EXTRACT(EPOCH FROM (x.first_contact - l.entered_pipeline_at)) / 60)
          FILTER (WHERE x.first_contact >= l.entered_pipeline_at)::numeric, 1),
    round(100.0 * count(*) FILTER (WHERE x.first_contact IS NOT NULL) / NULLIF(count(*), 0), 1)
  INTO primeiro_contato_min, taxa_contato_pct
  FROM crm_leads l
  CROSS JOIN LATERAL (
    SELECT LEAST(
