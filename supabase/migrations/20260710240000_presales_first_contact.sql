-- Adiciona "tempo de primeiro contato" (speed-to-lead): do lead entrar no funil
-- (entered_pipeline_at) até o PRIMEIRO toque, sendo o mais cedo entre: ligação do
-- discador, tarefa de ligação concluída, ou mensagem de WhatsApp enviada (outbound).
-- Média em minutos, sobre os leads que entraram no funil no período.
-- Índices já aplicados CONCURRENTLY: idx_crm_leads_entered_pipeline, idx_wa_conversations_lead
-- (+ idx_crm_calls_lead, idx_crm_activities_lead, idx_whatsapp_messages_conversation já existiam).
...
BEGIN
  ...
  -- Tempo de primeiro contato (min): entrada no funil -> primeiro toque (ligação/tarefa/WA out)
