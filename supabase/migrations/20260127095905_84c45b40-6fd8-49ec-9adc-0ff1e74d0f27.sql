-- Fix the badge check function to use correct column names
  ...
  FOR v_badge IN SELECT * FROM public.circle_badges WHERE is_active = true LOOP
    ...
    CASE v_badge.criteria_type
      ...
      WHEN 'likes_received' THEN
        -- circle_likes uses target_type and target_id
...
-- Also fix the trigger for likes
...
BEGIN
  IF NEW.target_type = 'post' THEN
    SELECT profile_id INTO v_post_owner FROM public.circle_posts WHERE id = NEW.target_id;
