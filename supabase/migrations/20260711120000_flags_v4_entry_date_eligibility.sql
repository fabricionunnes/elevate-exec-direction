-- v4: pessoa só ganha flag de mês fechado que trabalhou por inteiro (entrada <= dia 1º do mês)
-- clientes: exceção quando há lançamento de KPI no mês (dado retroativo importado/lançado vale)

create or replace function public.get_crm_staff_flags()
returns table(staff_id uuid, staff_name text, papel text, ref_month text, flag text, pct numeric, target_value numeric, achieved numeric, metric text)
language sql stable security definer set search_path to 'public'
as $function$
with months as (
  select to_char(date_trunc('month', (now() at time zone 'America/Sao_Paulo')) - make_interval(months => n), 'YYYY-MM') as my,
...
people as (
  select id, name, (created_at at time zone 'America/Sao_Paulo')::date entrada,
...
calc as (
  select p.id, p.name, p.papel, m.my,
         (p.entrada <= m.m_start) elig,
         g.meta_value target,
         case p.papel when 'sdr' then coalesce(s.n, 0) when 'head' then coalesce(tw.val, 0) else coalesce(w.val, 0) end got,
...
select c.id, c.name, c.papel, c.my,
       case when not c.elig then 'none'
       ...
       case when c.elig then round(c.got / nullif(c.target, 0) * 100, 1) end,
       case when c.elig then c.target end,
       case when c.elig then c.got end,
       c.metric
from calc c, allowed
where allowed.ok
...
create or replace function public.get_salesperson_flags(p_company_id uuid)
returns table(salesperson_id uuid, salesperson_name text, ref_month text, flag text, pct numeric, target_value numeric, achieved numeric, kpi_name text, inherited boolean)
language sql stable security definer set search_path to 'public'
as $function$
with months as (
  select to_char(date_trunc('month', (now() at time zone 'America/Sao_Paulo')) - make_interval(months => n), 'YYYY-MM') as my,
...
sellers as (
  select id, (created_at at time zone 'America/Sao_Paulo')::date entrada
  from company_salespeople where company_id = p_company_id and is_active = true
),
...
t_comp as (
  ...
  from kpi_monthly_targets t join months m on m.my = t.month_year
  where t.company_id = p_company_id and t.salesperson_id is null and t.team_id is null and t.unit_id is null and t.sector_id is null and t.target_value > 0
  order by m.my, t.kpi_id, (t.level_name = 'Meta') desc, t.level_order asc
),
t_default as (
  select k.id kpi_id, k.target_value target
  from company_kpis k
  where k.company_id = p_company_id and k.is_active = true and coalesce(k.target_value, 0) > 0 and k.salesperson_id is null
),
kpi_months as (
  select distinct k.kpi_id, m.my
  from months m cross join (select kpi_id from t_default union select kpi_id from t_ind union select kpi_id from t_comp) k
),
t_final as (
...
per_kpi as (
  select t.my, t.sid, k.name kname, t.target, t.inherited, coalesce(a.got, 0) got,
         ...
         case when k.kpi_type = 'monetary' then 1 when lower(k.name) ~ 'venda|fechamento' then 2 else 3 end prio
  from t_final t
...
select s.id, s.name, m.my,
       case when not elig.ok then 'none'
            when mk.sid is null then 'none'
            when mk.pct < 70 then 'red'
            when mk.pct <= 100 then 'yellow'
            else 'green' end,
       case when elig.ok then mk.pct end,
       case when elig.ok then mk.target end,
       case when elig.ok then mk.got end,
       case when elig.ok then mk.kname end,
       coalesce(mk.inherited, false)
from company_salespeople s
cross join months m
left join sellers se on se.id = s.id
cross join lateral (
  select (se.entrada <= m.m_start
          or exists (select 1 from kpi_entries e
                     where e.company_id = p_company_id and e.salesperson_id = s.id
                       and e.entry_date >= m.m_start and e.entry_date < m.m_end)) ok
) elig
